...
def main():
    ...
    for i in range(0, n_jugadores_por_equipo):
        jugadoresequipo1.append(Player("Jugador"+str(i), i, Coordinates(random.randint(
            0, field.width), random.randint(0, field.height)), field, color1, time))
    for i in range(0, n_jugadores_por_equipo):
        jugadoresequipo2.append(Player("Jugador"+str(i), i, Coordinates(random.randint(
            0, field.width), random.randint(0, field.height)), field, color2, time))
    equipo1 = Team(1, jugadoresequipo1, jugadoresequipo2, field, 1)
    equipo2 = Team(2, jugadoresequipo2, jugadoresequipo1, field, 2)

    screen = Screen(jugadoresequipo1+jugadoresequipo2,
