...
class Screen:
    ...
    def plot_players(self):
        for player in self.player_list:
            vertices_transformados = self.transform_polygon(player.action_area.exterior.coords)
            pygame.draw.polygon(self.screen, (255,0,0), vertices_transformados)
            
            pygame.draw.circle(self.screen,(0,0,255),self.transform_point(player.movement.move_destination_coordinates),10)


    def plot(self):
        # Método para actualizar la visualización
        self.screen.fill(self.bg_color)  # Rellenar el fondo con el color gris claro
