...
class Screen:
    ...
    def plot_players(self):
        for id in range(len(self.player_list)):
            vertices_transformados = self.transform_polygon(
                self.player_list[id].action_area.exterior.coords)
            ...
            if self.player_list[id].soycandidato == True:
                ...
                pygame.draw.polygon(
                    self.screen, (180, 180, 0), vertices_transformados)
                ...
                if self.player_list[id].fuicandidato > 0 and (self.time - self.player_list[id].fuicandidato) < 0.15:
                    ...
                    # print("me meteo")
                    pygame.draw.polygon(
                        self.screen, self.player_list[id].color, vertices_transformados)
                    self.player_list[id].fuicandidato = -1

            pygame.draw.circle(self.screen, (0, 0, 255), self.transform_point(
                self.player_list[id].movement.move_destination_coordinates), radius=2)
    ...
    def plot_field(self):
        vertices_transformados = self.transform_polygon(
            self.field.sites.area_grande_izquierda.exterior.coords)
        pygame.draw.polygon(self.screen, (255, 255, 255),
                            vertices_transformados, width=2)
        vertices_transformados = self.transform_polygon(
            self.field.sites.area_pequenia_izquierda.exterior.coords)
        pygame.draw.polygon(self.screen, (255, 255, 255),
                            vertices_transformados, width=2)
        vertices_transformados = self.transform_polygon(
            self.field.sites.area_grande_derecha.exterior.coords)
        pygame.draw.polygon(self.screen, (255, 255, 255),
                            vertices_transformados, width=2)
        vertices_transformados = self.transform_polygon(
            self.field.sites.area_pequenia_derecha.exterior.coords)
        pygame.draw.polygon(self.screen, (255, 255, 255),
                            vertices_transformados, width=2)
        vertices_transformados = self.transform_polygon(
            self.field.sites.mediocampo_izquierda.exterior.coords)
        pygame.draw.polygon(self.screen, (255, 255, 255),
                            vertices_transformados, width=2)
        vertices_transformados = self.transform_polygon(
            self.field.sites.mediocampo_derecha.exterior.coords)
        pygame.draw.polygon(self.screen, (255, 255, 255),
                            vertices_transformados, width=2)
        pygame.draw.circle(self.screen, (255, 255, 255), self.transform_point(
            [self.field.center_x, self.field.center_y]), radius=self.transform_polygon([[0, 9.15]])[0][1], width=2)

    def plot_ball(self):
        # Podriamos hacer la pelota más grande conforme se mueva hacia arriva hacia abajo
        vertices_transformados = self.transform_polygon(
            self.ball.action_area.exterior.coords)
    ...
    def plot_texto(self):
        ...
        texto = Math.convierteminutos(
            self.minutos) + ":" + segundos[0] + ":" + segundos[1]
        # Renderizar texto (antialias, color)
        renderizado_texto = fuente.render(
            texto, True, (255, 255, 255), (0, 0, 0))

        # Posicionar el texto
        posicion_texto = (self.view_width // 2 -
                          renderizado_texto.get_width() // 2, 10)
