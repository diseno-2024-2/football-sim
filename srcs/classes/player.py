...
class Alternativa():
    def __init__(self, player: "Player", tipo: 'AlternativaType', distancia_adversario: float, distancia_porteria: float, alternativas: List["Alternativa"] = None):
        self.player = player
...
class Player():
    ...
    def __init__(self, name: str, number: int, coordinates: Coordinates, field: Field, color, time: float):
        ...
        self.color = color
        self.fuerza_pase_corto = 13
        self.distancia_maxima_pase = math.pow(
            (self.fuerza_pase_corto * 0.5 / self.field.ball.masa), 2)/(-2 * (self.field.ball.movement.acceleration_frenado))
    ...
    def alternativasdemovimiento(self, posibilidades):
        posibilidades = 4
        grados_iteracion = 360.0/posibilidades
        print(grados_iteracion)
        vincial = [0.0, 1.0]
        alternativas = []
        for i in range(posibilidades):
            vrotado = Math.rotarvector(vincial, grados_iteracion * i) * \
                self.movement.distancia_maxima_de_movimiento_por_iteracion()
            alternativas.append(Alternativa(self, AlternativaType.MOVE, np.array([Math.distancia(
                self.coordinates.coordinates + vrotado, ri.coordinates.coordinates) for ri in self.rivales]).min(), Math.distancia(self.coordinates.coordinates + vrotado, self.coordenadas_porteria_rival)))
        return alternativas
    ...
    def pasar_balon_random(self):
        ...
        arbol: List[Alternativa] = arbol_alternativas(self, 5)
        ...
        if (len(arbol) > 0):
            ...
            if arbol[elegido].tipo == AlternativaType.PASS:
                # print("estoy en PASS")
                direccion = arbol[elegido].player.coordinates.coordinates - \
                    self.coordinates.coordinates
                self.info_equipo.balon_pasado_a = arbol[elegido].player.id
                self.info_equipo.balon_pasado_a_jugador = arbol[elegido].player
                # self.doing_something = self.field.ball.pass_ball(
                #   direccion, self.fuerza_pase_corto, self.id)
                self.field.ball.pass_ball(
                    direccion, self.fuerza_pase_corto, self.id)
                self.tengoelbalon = False
                # print("Termino de pasar")
            else:
                # print("estoy aqui")
                self.movement.move_destination_coordinates = arbol[
                    elegido].player.coordinates.coordinates
                self.movement.move()
    ...
    def cubre(self):
        ...
        if len(self.jugadores_rivales_en_mi_zona) > 0:
            ...
            self.movement.move_destination_coordinates = self.jugadores_rivales_en_mi_zona[
                id] + vectorcubrir
...
def altenativas_recursivo(alternativa: Alternativa, profundidad, jugadoresvetados: List[int] = []):
    if profundidad >= 0:
        alternativa.alternativas = alternativa.player.alternativas_de_pase_disponibles(
            jugadoresvetados)
        if profundidad <= 4:
...
def arbol_alternativas(player: Player, profundidad: int):
    ...
    # + player.alternativasdemovimiento(10)
    altenativas = player.alternativas_de_pase_disponibles_iluminarcandidatos()
...
def representar_arbol_alternativas(alternativas: List[Alternativa], profundidad=0):
    ...
    if (alternativas != None):
        for alternativa in alternativas:
            print("Profundidad ", profundidad, ": ", alternativa)
            representar_arbol_alternativas(
                alternativa.alternativas, profundidad+1)
...
def eligealtenativa_recursivo(alternativas: List[Alternativa], sumatorio: float = 0.0, profundidad: float = 0.0):
    if alternativas == None or len(alternativas) == 0:
        return (sumatorio/profundidad, 0)
    else:
        resultados = []
        for id in range(len(alternativas)):
            media, idseleccionada = eligealtenativa_recursivo(
                alternativas[id].alternativas, sumatorio + alternativas[id].distancia_adversario, profundidad + 1)
    ...
    def distancia_maxima_de_movimiento_por_iteracion(self):
       ...
       # print("Tiempo Restante Para LLegar: ",tiemporestante, " Distancia Máxima de Frenado:  ", distancia_maxima_frenado, " Aceleracion Necesaria: ", aceleracion_necesaria)
        ...
        if (distancia_maxima_frenado > self.destination_distance and (self.destination_distance > 0.1)):
            if (aceleracion_necesaria < current_acceleration):
                if aceleracion_necesaria < self.maximum_acceleration_braking:
                    current_acceleration = self.maximum_acceleration_braking
                else:
                    current_acceleration = aceleracion_necesaria

        v0 = self.current_vel

        current_vel = self.current_vel + \
            (current_acceleration * self.player.time)

        if (current_vel < 0):
            current_acceleration = self.acceleration * \
                (1 - (current_vel/self.sprint_speed))
    ...
    def runwithball(self):
       ...
       # print("Tiempo Restante Para LLegar: ",tiemporestante, " Distancia Máxima de Frenado:  ", distancia_maxima_frenado, " Aceleracion Necesaria: ", aceleracion_necesaria)
        ...
        self.current_vel = self.current_vel + \
            (self.current_acceleration * self.player.time)
        ...
        if self.destination_distance > space:
            ...
            self.player.coordinates.coordinates = self.player.coordinates.coordinates + \
                (self.move_destination_coordinates -
