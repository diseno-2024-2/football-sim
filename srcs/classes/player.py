...
class Player():
    ...
    def __init__(self, name: str, number: int, coordinates: Coordinates,field: Field,color,time: float):
        ...
        self.distancia_maxima_pase =  math.pow((self.fuerza_pase_corto* 0.5 / self.field.ball.masa),2)/(-2 * (self.field.ball.movement.acceleration_frenado))    
        ...
        if self.id_equipo == 0:
            self.coordenadas_porteria_rival = [self.field.width,self.field.center_y]
        else: 
    ...
    def alternativasdemovimiento(self,posibilidades):
        ...
        for i in range(posibilidades):
            vrotado = Math.rotarvector(vincial,grados_iteracion * i) * self.movement.distancia_maxima_de_movimiento_por_iteracion()
            alternativas.append(Alternativa(self,AlternativaType.MOVE, np.array([Math.distancia(self.coordinates.coordinates + vrotado,ri.coordinates.coordinates) for ri in self.rivales]).min() ,Math.distancia(self.coordinates.coordinates + vrotado,self.coordenadas_porteria_rival)))
        return alternativas
    ...
    def pasar_balon_random(self):
        ...
        if(len(arbol) > 0):
            ...
            if arbol[elegido].tipo == AlternativaType.PASS:
                ...
                self.movement.move_destination_coordinates = arbol[elegido].player.coordinates.coordinates
                self.movement.move()
...
def arbol_alternativas(player: Player,profundidad:int):
    ...
    altenativas = player.alternativas_de_pase_disponibles_iluminarcandidatos() #+ player.alternativasdemovimiento(10)
...
def eligealtenativa_recursivo(alternativas: List[Alternativa],sumatorio:float = 0.0,profundidad:float = 0.0):
    if alternativas == None or len(alternativas) == 0:
        ...
        for id in range(len(alternativas)):
            media,idseleccionada = eligealtenativa_recursivo(alternativas[id].alternativas,sumatorio + alternativas[id].distancia_adversario,profundidad + 1)
