...
class Probabilidades():
    pesos_decision_alternativas_equipo_perdiendo: np.array = np.array([0.41,0.32,0.27])
    pesos_decision_alternativas_equipo_ganando: np.array = np.array([0.37,0.21,0.42])
...
class Player():
    ...
    def __init__(self, name: str, number: int, coordinates: Coordinates, field: Field, color, time: float):
        ...
        self.distancia_maxima_pase_largo = math.pow((self.fuerza_maxima_pase_largo* self.field.ball.tiempo_contacto / self.field.ball.masa),2)/(9.8)  
        self.distancia_maxima_pase =  math.pow((self.fuerza_pase_corto* self.field.ball.tiempo_contacto / self.field.ball.masa),2)/(-2 * (self.field.ball.movement.acceleration_frenado))    
